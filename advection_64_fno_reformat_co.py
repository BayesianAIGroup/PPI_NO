...
def pre_train_fu(Ntr, file_number, relative_fno_model_directory):
    ...
    np.save(f'pre_res/advection_64_predict_fno_res_{Ntr}_{file_number}.npy', f_aug_fno)

    # file_path = os.path.join(relative_fno_model_directory, f'darcy_128_{Ntr}_fno_model_{file_number}.pth')
    file_path = 'fno_advection_64_f_2_u_model.pth'
    torch.save({
...
def train_PPI_f_u_model(Ntr, loop_num, pre_fno_error_list, ppi_fno_error_list, epochs, lam, batch_size, model_directory,
    ...
    # load fu
    model_fu = FNO2d(modes, modes, width, s).float().cuda()
    checkpoint_fu = torch.load('fno_advection_64_f_2_u_model.pth')
    ...
    if file_num == 5 and loop_num == num_iteration -1:
        ...
        np.save(f'advection_64_data/pre_res/advection_64_ppi_fno_predict_res_{Ntr}_{file_num}.npy', predict_result_cpu)
        del predict_result
...
if __name__ == '__main__':
    ...
    #Ntr_list = [10, 20, 30, 50, 80]  # Define your training sizes
    Ntr_list = [20,30,50,80]
    file_list = [5]
    epochs = 1000
    batch_size = 10
    lam = 20
    ...
    print("20,30,50,80 advection_64 fno_pretrain mean error:", pre_fno_mean_record)
    print("20,30,50,80 advection_64 fno_pretrain std error:", pre_fno_std_record)
