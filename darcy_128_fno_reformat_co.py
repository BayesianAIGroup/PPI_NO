...
def get_data(Ntr, file_number, with_sampled):
    ...
    x_cor = np.expand_dims(grid[0], axis=0)  # 1 x 128 x 128
...
def pre_train_fu(Ntr, file_number, relative_fno_model_directory):
    ...
    total_params = sum(p.numel() for p in model_fu.parameters())
    ...
    for ep in range(epochs):
        ...
        for x, y in train_loader:
            ...
            loss_fu = myloss(out.reshape(batch_size, -1), y.reshape(batch_size, -1))
            loss_fu.backward()
            optimizer_fu.step()
    ...
    plt.grid(True)
    plt.savefig("darcy_training_test_loss_trend.pdf", format='pdf')  # Save the plot as an image
    #plt.show()
...
if __name__ == '__main__':
    ...
    print("5, 10, 20, 30 darcy_128 fno_pretrain mean error:", pre_fno_mean_record)
    print("5, 10, 20, 30 darcy_128 fno_pretrain std error:", pre_fno_std_record)
