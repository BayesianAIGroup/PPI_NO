...
class DeepONet(StructureNN):
    ...
    def forward(self, x):
        ...
        x_branch = x_branch.unsqueeze(1).repeat(1, 128, 1)
        #x_branch = x_branch.unsqueeze(1).repeat(1, 128, 1, 1)
...
class MLP(nn.Module):
    def __init__(self, layers):
        ...
        for j in range(len(layers) - 1):
            self.mlp.append(nn.Conv1d(layers[j], layers[j + 1], 1))
...
class FWD2d(nn.Module):
    def __init__(self, input_channel, channel_num, layers):
        ...
        self.conv = nn.Conv1d(input_channel, self.channel_num, kernel_size=3, padding='same', stride=1, padding_mode='zeros')

        self.layers = [channel_num] + layers + [3]
...
def get_data(Ntr, file_number, with_sampled):
    data_x_train = np.load(f'crack_data/train/crack_128_{Ntr}_train_f_{file_number}.npy', allow_pickle=True)
    data_x_test = np.load(f'crack_data/test/X_test.npy', allow_pickle=True)
    data_y_train = np.load(f'crack_data/train/crack_128_{Ntr}_train_u_{file_number}.npy', allow_pickle=True)
    data_y_test = np.load(f'crack_data/test/y_test.npy', allow_pickle=True)
    # data_x_train = np.load(f'FNO_data/X_train.npy', allow_pickle=True)
    # data_x_test = np.load(f'FNO_data/X_test.npy', allow_pickle=True)
    # data_y_train = np.load(f'FNO_data/y_train.npy', allow_pickle=True)
    # data_y_test = np.load(f'FNO_data/y_test.npy', allow_pickle=True)
    print(data_x_train.shape, data_x_test.shape, data_y_train.shape, data_y_test.shape)
    # print(data_train[0].shape)
    f_tr = data_x_train[0:Ntr]
    f_te = data_x_test[-200:]
    u_tr = data_y_train[0:Ntr]
    u_te = data_y_test[-200:]
    sampled_num = 0
    if with_sampled:
    ...
    grid = np.meshgrid(np.linspace(0, 1, u_tr.shape[1]))
    x_cor = np.expand_dims(grid[0], axis=0)  # 1 x 256 x 256
    x_cor = np.repeat(x_cor, u.shape[0], axis=0)
    X = np.stack((u, ux, uxx, x_cor), axis=-1)  # (Ntr+Nte) x 256 x 256
...
def load_eq_simple_data(Ntr, file_number):
    ...
    print(data_x_train.shape, data_x_test.shape, data_y_train.shape, data_y_test.shape)
    # print(data_train[0].shape)
    x_train = data_x_train[0:Ntr]
    x_test = data_x_test[-200:]
    y_train = data_y_train[0:Ntr]
    y_test = data_y_test[-200:]
...
def pre_train_fu(Ntr, file_number, relative_fno_model_directory):
    ...
    x_train = torch.tensor(x_train[:,:,0:3], device="cuda", dtype=torch.float)
    x_test = torch.tensor(x_test[:,:,0:3], device="cuda", dtype=torch.float)
    ...
    f_aug = f_aug.reshape(test_data_number, s)
    ...
    f_aug_fno = f_aug.cpu().detach().numpy().reshape(test_data_number, s)
    np.save(f'pre_res/crack_128_predict_fno_res_{Ntr}_{file_number}.npy', f_aug_fno)

    # file_path = os.path.join(relative_fno_model_directory, f'darcy_128_{Ntr}_fno_model_{file_number}.pth')
    file_path = 'deeponet_crack_128_f_2_u_model.pth'
    torch.save({
...
def pre_train_uf(Ntr, file_number, with_sampled):
    ...
    model_uf = FWD2d(4, channel_num, NN_layers).float().cuda()
    learning_rate = 1e-3
    ...
    for ep in range(epochs):
        ...
        for x, y in train_loader:
            ...
            out = model_uf(x)
            #print(out.shape, out[:,:,0].shape, y[:,:,0].shape)
            loss_uf1 = myloss(out[:,:,0].reshape(batch_size, -1), y[:,:,0].reshape(batch_size, -1))
            loss_uf2 = myloss(out[:, :, 1].reshape(batch_size, -1), y[:, :, 1].reshape(batch_size, -1))
            loss_uf3 = myloss(out[:, :, 2].reshape(batch_size, -1), y[:, :, 2].reshape(batch_size, -1))
            loss_uf = (loss_uf1 + loss_uf2 + loss_uf3)/3
            loss_uf.backward()
            optimizer_uf.step()
        ...
        with torch.no_grad():
            for x, y in test_loader:
                ...
                loss_test_uf1 = myloss(out[:, :, 0].reshape(batch_size, -1), y[:, :, 0].reshape(batch_size, -1))
                loss_test_uf2 = myloss(out[:, :, 1].reshape(batch_size, -1), y[:, :, 1].reshape(batch_size, -1))
                loss_test_uf3 = myloss(out[:, :, 2].reshape(batch_size, -1), y[:, :, 2].reshape(batch_size, -1))
                loss_test_uf = (loss_test_uf1 + loss_test_uf2 + loss_test_uf3) / 3

                loss_test += loss_test_uf
                total_test_uf1 += loss_test_uf1
                total_test_uf2 += loss_test_uf2
                total_test_uf3 += loss_test_uf3
        ...
        print(ep, 'train l2=', train_l2, 'test loss =', loss_test, 'channle one loss=', total_test_uf1, 'channle two loss=', total_test_uf2, 'channle three loss=', total_test_uf3)
...
def prepare_data(Ntr, file_num, s, normalizer_class, gen_num):
    ...
    x_train, y_train = torch.tensor(x_train[:,:,0:3], device=device), torch.tensor(y_train, device=device)
    x_test, y_test = torch.tensor(x_test[:,:,0:3], device=device), torch.tensor(y_test, device=device)
    ...
    f_ori = np.load(f'crack_data/train/crack_sampled_f.npy', allow_pickle=True)
    f_ori = torch.tensor(f_ori[0:gen_num,:,0:3], device="cuda", dtype=torch.float)
    f_ori = f_ori.cuda()
...
def train_PPI_f_u_model(Ntr, loop_num, pre_fno_error_list, ppi_fno_error_list, total_best_error, epochs, lam, batch_size, model_directory,
    ...
    if loop_num == 0:
        ...
        print("best error", pretrain_error)
        pre_fno_error_list.append(pretrain_error)
        pre_train_uf(Ntr, file_num, False)

    # load uf
    model_uf = FWD2d(4, channel_num, NN_layers).float().cuda()
    checkpoint_uf = torch.load('crack_128_u_2_f_model.pth')
    ...
    model_fu = DeepONet(branch_dim, trunk_dim).float().cuda()
    checkpoint_fu = torch.load('deeponet_crack_128_f_2_u_model.pth')
    ...
    for ep in range(epochs):
        ...
        grid = np.meshgrid(np.linspace(0, 1, u1.shape[1]), np.linspace(0, 1, u1.shape[1]))
        x_cor = np.repeat(np.linspace(0, 1, u1.shape[1]).reshape(1,s), u1.shape[0], axis=0)
        x_cor = torch.tensor(x_cor, dtype=torch.float, device='cuda')
        X_aug = torch.stack((u1, ux[0], uxx[0], x_cor), axis=-1)

        X_aug = 2.0 * (X_aug - lb) / (ub - lb) - 1.0
        X_aug = X_aug.permute(0, 2, 1).to(torch.float)
        ...
        if is_fixing_uf:
            ...
            f_pred = model_uf(xtr)
            loss_train1 = myloss(f_pred[:, :, 0].reshape(train_data_number, -1),
                           ytr[:, :, 0].reshape(train_data_number, -1)) / train_data_number
            loss_train2 = myloss(f_pred[:, :, 1].reshape(train_data_number, -1),
                           ytr[:, :, 1].reshape(train_data_number, -1)) / train_data_number
            loss_train3 = myloss(f_pred[:, :, 2].reshape(train_data_number, -1),
                           ytr[:, :, 2].reshape(train_data_number, -1)) / train_data_number
            loss1 = (loss_train1 + loss_train2 + loss_train3)/3

        loss_uf1 = myloss(f_renew[:, :, 0].reshape(gen_num, -1), sampled_f_ori[:, :, 0].reshape(gen_num, -1))
        loss_uf2 = myloss(f_renew[:, :, 1].reshape(gen_num, -1), sampled_f_ori[:, :, 1].reshape(gen_num, -1))
        loss_uf3 = myloss(f_renew[:, :, 2].reshape(gen_num, -1), sampled_f_ori[:, :, 2].reshape(gen_num, -1))

        loss_uf = (loss_uf1 + loss_uf2 + loss_uf3) / 3
        loss2 = lam * loss_uf / gen_num
        #print(loss2, lam)
        loss = loss1 + loss2
        #print(loss)
        ...
        if is_fixing_uf:
            ...
            with torch.no_grad():
                y_pred = model_uf(xte)
                error_ch1 = myloss(y_pred[:, :, 0].reshape(200, -1), yte[:, :, 0].reshape(200, -1)).item()
                error_ch2 = myloss(y_pred[:, :, 1].reshape(200, -1), yte[:, :, 1].reshape(200, -1)).item()
                error_ch3 = myloss(y_pred[:, :, 2].reshape(200, -1), yte[:, :, 2].reshape(200, -1)).item()
                err = (error_ch1 + error_ch2 + error_ch3)/3
                rel_err += err
    ...
    file_path = 'deeponet_crack_128_f_2_u_model.pth'
    uf_file_path = 'crack_128_u_2_f_model.pth'
    if is_fixing_uf:
        torch.save({
    ...
    if loop_num %2 == 0:
        if total_best_error is None or total_best_error > best_err:
            total_best_error = best_err
    if loop_num  == num_iteration - 1:
        ppi_fno_error_list.append(total_best_error)


    print("total best error", total_best_error)
    # if file_num == 5 and loop_num == 8:
    #     predict_result = y_normalizer.decode(model_fu(x_test))
    #     predict_result_cpu = predict_result.cpu().detach().numpy()
    #     np.save(f'crack_data/pre_res/crack_ppi_fno_predict_res_{Ntr}_{file_num}.npy', predict_result_cpu)
    #     del predict_result
    #     del predict_result_cpu


def setup_directories(base_path='crack_data/fno_model'):
...
if __name__ == '__main__':
    ...
    Ntr_list = [400, 500,600]  # Define your training sizes
    file_list = [5]
    epochs = 1000
    batch_size = 10
    lam = 20
        ...
        for file_num in file_list:
            ...
            for i in range(num_iteration):
                if i % 2 == 0:
                    ...
                    lam = 20
                    epochs = 300
                train_PPI_f_u_model(Ntr, i, cum_pre_fno_error, cum_err, total_best_error, epochs=epochs, lam=lam, batch_size=batch_size,
    ...
    print("400, 500, 600 crack_128 deeponet_pretrain mean error:", pre_fno_mean_record)
    print("400, 500, 600 crack_128 deeponet_pretrain std error:", pre_fno_std_record)
    print("400, 500, 600 crack_128 joint_training mean error:", total_mean_record)
    print("400, 500, 600 crack_128 joint_training std error:", total_std_record)
