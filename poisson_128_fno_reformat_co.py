...
def get_data(Ntr, file_number, with_sampled):
    data_train = np.load(f'poisson_data/train/poisson_128_{Ntr}_train_{file_number}.npy', allow_pickle=True)
    data_test = np.load(f'poisson_data/test/poisson_128_test.npy', allow_pickle=True)
...
def pre_train_fu(Ntr, file_number, relative_fno_model_directory):
    ...
    print(f"pretrain runtime: {pretrain_end_time - pretrain_start_time:.2f} seconds")

    plt.figure(figsize=(10, 6))
    plt.plot(range(1, epochs + 1), test_loss_list, marker='x')
    plt.xlabel("Epochs", fontsize=20)
    plt.ylabel("Relative L2 error", fontsize=20)
    #plt.title("darcy data FNO Test Loss Trends")
    #plt.legend(fontsize=24)
    plt.grid(True)
    plt.savefig("poisson_fno_test_loss_trend.pdf", format='pdf' )  # Save the plot as an image
    #plt.show()
    ...
    #np.save(f'pre_res/poisson_128_predict_fno_res_{Ntr}_{file_number}.npy', f_aug_fno)

    # file_path = os.path.join(relative_fno_model_directory, f'darcy_128_{Ntr}_fno_model_{file_number}.pth')
    file_path = 'fno_poisson_128_f_2_u_model.pth'
    torch.save({
...
def prepare_data(Ntr, file_num, s, normalizer_class, gen_num):
    ...
    f_ori = np.load(f'poisson_data/train/poisson_sampled_f.npy', allow_pickle=True)
...
def train_PPI_f_u_model(Ntr, loop_num, pre_fno_error_list, ppi_fno_error_list, epochs, lam, batch_size, model_directory,
    ...
    # load uf
    model_uf = FWD2d(7, channel_num, NN_layers).float().cuda()
    checkpoint_uf = torch.load('poisson_128_u_2_f_model.pth')
    ...
    # load fu
    model_fu = FNO2d(modes, modes, width, s).float().cuda()
    checkpoint_fu = torch.load('fno_poisson_128_f_2_u_model.pth')
    ...
    file_path = 'fno_poisson_128_f_2_u_model.pth'
    uf_file_path = 'poisson_128_u_2_f_model.pth'
    if is_fixing_uf:
        torch.save({
            'model_state_dict': model_fu.state_dict(),
        }, file_path)
    ...
    #     np.save(f'poisson_data/pre_res/poisson_ppi_fno_predict_res_{Ntr}_{file_num}.npy', predict_result_cpu)
    #     del predict_result
    #     del predict_result_cpu


def setup_directories(base_path='poisson_data/fno_model'):
...
if __name__ == '__main__':
    ...
    print("5,10,20,30 poisson_128 fno_pretrain mean error:", pre_fno_mean_record)
    print("5,10,20,30 poisson_128 fno_pretrain std error:", pre_fno_std_record)
    print("5,10,20,30 poisson_128 joint_training mean error:", total_mean_record)
    print("5,10,20,30 poisson_128 joint_training std error:", total_std_record)
    end_time = time.time()
